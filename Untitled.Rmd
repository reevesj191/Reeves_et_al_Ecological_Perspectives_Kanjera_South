...
```{r cores prep}
cores <- read.csv("Data/kjs_cores.csv")
...
cores$DeLaTorre.Typology <- factor(cores$DeLaTorre.Typology, 
...
kable(cores_table,caption = "Table 2: A Summary of the cores included in the analysis by raw material")
...
kable(flakes_table, caption = "Table 3: A Summary of flakes included in the Study by raw material")
```
...
```{r}
wilcox.test(flakes$flake.seq~
             flakes$rm.provenance)
```
```{r fig-5, echo=FALSE,dpi= 1000,fig.height= 3, fig.width= 6, fig.cap= Left: "Boxplots of the measures of flake efficiency. Y-axis represents perimeter of flakes divided by a logarithmically transformed mass value. Right: A scatter plot examining the relationship between flake efficiency and flake sequence. \\label{flake_efficiency}"}
...
## Significance of core reduction intensity versus core reduction strategies
```{r}
...
ggplot(flakes, aes(x = flake.seq, y = PerMass)) + geom_point()

summary(lm(flakes$PerMass~flakes$flake.seq + flakes$Raw.Material))
